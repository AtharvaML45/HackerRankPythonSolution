...
[lst.append([i, j, k]) for i in range(x+1) for j in range(y+1) 
...
```
Explanation:
1) We have given x , y,z intergeres wich are also inclusive for that purpose we are taking range for iteration in x+1 
2) By iteration in loops we check if condition i + j+ k != n is true of not then append the elements to to list.
3) In list comprehensions we use all the loops and condition within single parentheses.
